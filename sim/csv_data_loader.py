...
class CSVDataLoader:
    ...
    def load(self, max_rows: Optional[int] = None) -> pd.DataFrame:
        ...
        try:
            raw_data = pd.read_csv(self.csv_path, nrows=max_rows)
            self.data = self._canonicalize_dataset(raw_data)
    ...
    @staticmethod
    def _parse_vector_column(value) -> np.ndarray:
        """Parse a serialized state vector into a numeric numpy array."""
    ...
    @staticmethod
    def _series_to_numeric(series: Optional[pd.Series], default: float, index: pd.Index) -> pd.Series:
        if series is None:
            return pd.Series(default, index=index, dtype=np.float64)
        return pd.to_numeric(series, errors="coerce").fillna(default)

    def _canonicalize_dataset(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Canonicalize input CSV to the schema expected by training utilities.

        Supports both:
        - ESA-style datasets (already canonical columns)
        - Synthetic CDM CSV generated by data/generate_cdm_dataset.py
        """
        required = {"event_id", "time_to_tca", "risk", "miss_distance", "relative_speed"}
        ...
        if not synthetic_signature.issubset(df.columns):
            missing = sorted(required - set(df.columns))
            raise ValueError(
                f"Unsupported dataset schema. Missing canonical columns: {missing}. "
                "Expected either ESA canonical columns or synthetic CDM columns."
            )

        canonical = df.copy()

        canonical["event_id"] = canonical["cdm_id"].astype(str)
        epoch_ts = pd.to_datetime(canonical["epoch_utc"], utc=True, errors="coerce")
        tca_ts = pd.to_datetime(canonical["TCA_utc"], utc=True, errors="coerce")
        time_to_tca_h = (tca_ts - epoch_ts).dt.total_seconds() / 3600.0
        canonical["time_to_tca"] = time_to_tca_h.fillna(1.0).clip(lower=0.01)

        collision_prob = self._series_to_numeric(
            canonical["collision_probability"], default=1e-6, index=canonical.index
        ).clip(
            lower=1e-12, upper=1.0
        )
        # Match existing risk-threshold flow that expects larger values for higher risk.
        canonical["risk"] = np.log10(collision_prob)
        canonical["miss_distance"] = self._series_to_numeric(
            canonical["miss_distance_m"], default=1000.0, index=canonical.index
        ).abs()

        if {"state_vector_primary", "state_vector_secondary"}.issubset(canonical.columns):
            primary_vec = canonical["state_vector_primary"].apply(self._parse_vector_column)
            secondary_vec = canonical["state_vector_secondary"].apply(self._parse_vector_column)
            ...
            for p_vec, s_vec in zip(primary_vec, secondary_vec):
                p = np.pad(np.asarray(p_vec, dtype=np.float64), (0, max(0, 6 - len(p_vec))))[:6]
                s = np.pad(np.asarray(s_vec, dtype=np.float64), (0, max(0, 6 - len(s_vec))))[:6]
                d_pos = s[:3] - p[:3]
                d_vel = s[3:6] - p[3:6]
                rel_pos_r.append(float(d_pos[0]))
                rel_pos_t.append(float(d_pos[1]))
                rel_pos_n.append(float(d_pos[2]))
                rel_vel_r.append(float(d_vel[0]))
                rel_vel_t.append(float(d_vel[1]))
                rel_vel_n.append(float(d_vel[2]))
                rel_speed.append(float(np.linalg.norm(d_vel)))

            canonical["relative_position_r"] = rel_pos_r
            canonical["relative_position_t"] = rel_pos_t
            canonical["relative_position_n"] = rel_pos_n
            canonical["relative_velocity_r"] = rel_vel_r
            canonical["relative_velocity_t"] = rel_vel_t
            canonical["relative_velocity_n"] = rel_vel_n
            canonical["relative_speed"] = np.maximum(np.asarray(rel_speed, dtype=np.float64), 1.0)
        else:
            canonical["relative_position_r"] = canonical["miss_distance"]
            canonical["relative_position_t"] = 0.0
            canonical["relative_position_n"] = 0.0
            canonical["relative_velocity_r"] = 0.0
            canonical["relative_velocity_t"] = 7500.0
            canonical["relative_velocity_n"] = 0.0
            canonical["relative_speed"] = 7500.0

        canonical["t_j2k_sma"] = self._series_to_numeric(
            canonical.get("primary.orbit.a_km"), default=7000.0, index=canonical.index
        )
        canonical["t_j2k_ecc"] = self._series_to_numeric(
            canonical.get("primary.orbit.e"), default=0.001, index=canonical.index
        ).clip(
            lower=0.0, upper=0.2
        )
        canonical["t_j2k_inc"] = self._series_to_numeric(
            canonical.get("primary.orbit.i_deg"), default=98.0, index=canonical.index
        )

        sma_offset = (canonical["miss_distance"] / 1000.0).clip(lower=0.001, upper=20.0)
        canonical["c_j2k_sma"] = canonical["t_j2k_sma"] + sma_offset
        canonical["c_j2k_ecc"] = (canonical["t_j2k_ecc"] + 1e-5).clip(lower=0.0, upper=0.2)
        canonical["c_j2k_inc"] = canonical["t_j2k_inc"] + 0.01
